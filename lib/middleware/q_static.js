...
var comboRead = function(ctx, urls) {
  ...
  return function*() {
    ...
    var isSupportCombo = function(_url){
      var pathname = urlUtil.parse(_url).pathname;
      var extname = path.extname(pathname).substring(1);
      return ~ctx.md_config.comboType.split('|').indexOf(extname);
    }
    for (; i < l; i++) {
      yield send(ctx, urlUtil.parse(urls[i]).pathname, {
...
exports.service = function*(next) {
  ...
  if (comboIndex === -1) {
    ...
    urls = url.substring(comboIndex + prefixLength).split(',').map(function(u) {
      ...
      var pname = urlUtil.parse(url).pathname || '/';
      return path.resolve(pname, u);
    });
